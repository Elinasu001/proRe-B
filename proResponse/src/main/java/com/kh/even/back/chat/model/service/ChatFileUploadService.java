...
public class ChatFileUploadService {
	 ...
	 */
	public void uploadFiles(List<MultipartFile> files, String directory, Long messageNo,
			Consumer<ChatAttachmentVO> saveCallback) {
		...
		for (MultipartFile file : files) {
			...
			ChatAttachmentVO vo = ChatAttachmentVO.builder().originName(file.getOriginalFilename()).filePath(filePath).status("Y").messageNo(messageNo)
					.build();
