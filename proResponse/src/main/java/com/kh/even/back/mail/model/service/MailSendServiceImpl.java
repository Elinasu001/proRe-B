...
public class MailSendServiceImpl implements MailSendService {
	...
	@Override
	public void sendTempPassword(String toEmail, String title, String tempPwd) {
		...
		try {
			...
			String html =
	                "<div style='font-family: Arial, sans-serif; line-height: 1.6;'>"
	              + "  <h2 style='margin:0 0 8px;'>임시 비밀번호 안내</h2>"
	              + "  <p style='margin:0 0 16px;'>임시 비밀번호로 로그인 하신 뒤에 비밀번호를 변경해주세요.</p>"
	              + "  <div style='display:inline-block; padding:12px 16px; border:1px solid #ddd; border-radius:8px;'>"
	              + "    <span style='font-size:28px; font-weight:700; letter-spacing:6px;'>" + tempPwd + "</span>"
	              + "  </div>"
	              + "</div>";
