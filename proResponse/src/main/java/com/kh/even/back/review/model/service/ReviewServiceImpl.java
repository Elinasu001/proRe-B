...
public class ReviewServiceImpl implements ReviewService {
	...
	public ReviewVO saveReview(ReviewDTO reviewDTO, List<MultipartFile> files, Long userNo) {
		...
		// 2. 리뷰 등록
		ReviewVO reviewVO = ReviewVO.builder().estimateNo(reviewDTO.getEstimateNo()).content(reviewDTO.getContent())
				.starScore(reviewDTO.getStarScore()).status("Y").build();
		...
		if (reviewDTO.getTagNos() != null && !reviewDTO.getTagNos().isEmpty()) {
			for (Long tagNo : reviewDTO.getTagNos()) {
				ReviewMapVO mapVO = ReviewMapVO.builder().reviewNo(reviewNo).tagNo(tagNo).build();
		...
		// log.info("리뷰 등록 완료 - reviewNo: {}, estimateNo: {}, 태그수: {}", reviewNo,
		// reviewDTO.getEstimateNo(), reviewDTO.getTagNos() != null ?
		// reviewDTO.getTagNos().size() : 0);
		return reviewVO;
	}

	private void saveAttachments(List<MultipartFile> files, Long reviewNo) {
		AssertUtil.validateImageFiles(files);
		fileUploadService.uploadFiles(files, "reviews", reviewNo, fileVO -> {

			ReviewAttachmentVO attachmentVO = ReviewAttachmentVO.builder().reviewNo(reviewNo)
					.originName(fileVO.getOriginName()).filePath(fileVO.getFilePath()).status("Y").build();
